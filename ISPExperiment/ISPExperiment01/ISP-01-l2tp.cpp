...
#define SUBSTITUTE_LEN 36
char SubString[SUBSTITUTE_LEN] = "I LOVE Information Safety Principle";
...
int decode_l2tp(char* l2tpbuf)
{
    ...
    printf("隧道标识符      :%d\n", ntohs(pl2tpheader->TunnelId));
    printf("会话标识符      :%d\n", ntohs(pl2tpheader->SessionId));
    if (s == 1) { // 顺序字段在位标志为1
...
int decode_udp(char* udpbuf)
{
    ...
    if (ntohs(pudpheader->sport) == 4500 && ntohs(pudpheader->dport) == 4500) {
        ...
        decode_l2tp((char*)(udpbuf + UDP_HEADER_LEN));
    } 
    return true;
}
...
int decode_ethernet(char* etherbuf)
{
    ...
    printf("\n");
    if (type == 0x0800) {
        decode_ipv4((char*)(etherbuf + ETHERNET_HEADER_LEN));
    }
    return true;
...
void packet_handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data)
{
    ...
    //对静载荷进行替换
    for (int i = TOTAL_HEADER_LEN; i < TOTAL_HEADER_LEN + SUBSTITUTE_LEN; i++) {
        mypkt[i - 1] = SubString[i - TOTAL_HEADER_LEN];
    }

    printf("对静载荷进行替换后：\n");
    for (int i = 1; i < TOTAL_HEADER_LEN + SUBSTITUTE_LEN; i++)
    {
...
int main(int argc, const char* argv[])
{
    ...
    u_int netmask;
    //服务器IP：120.26.128.205
    // 手机IP：192.168.137.176
    char packet_filter[] = "src host 192.168.137.176 or dst host 192.168.137.176";
